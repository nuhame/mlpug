...
class NTPTrainingProcess(TrainingProcess):
    ...
    def _apply_subsampling(
        ...
        :param fraction: Fraction of data to use (0-1), or None for all data.
        :param name: Dataset name for logging (e.g., "training", "validation").

        :return: Original dataset or SubsampledDataset wrapper.
        """
        if fraction is None:
            return dataset
        ...
        if self._seed is None:
            raise ValueError(
                f"{name}_fraction requires a seed for reproducible subsampling across "
                "distributed processes. Please specify --seed."
            )

        self._log.info(f"  Subsampling {name} data to {fraction:.1%} (seed={self._seed})")
        return SubsampledDataset(dataset, subsample_factor=fraction, seed=self._seed)
