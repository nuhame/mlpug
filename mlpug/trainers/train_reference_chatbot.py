...
from pytorch.trainers.training import TrainingManager, DefaultTrainer

from pytorch.trainers.callbacks.callback import Callback
from pytorch.trainers.callbacks.basic import LogProgress, BatchSizeLogger
from pytorch.trainers.callbacks.metrics_logger import TestMetricsLogger
from pytorch.trainers.callbacks.checkpoint_manager import CheckpointManager
from pytorch.trainers.callbacks.tensorboard import Tensorboard, AutoTensorboard

from utils import get_value_at

from basics.logging import get_logger

parser = argparse.ArgumentParser(description='Train reference chatbot on processed sentence pair data, as generated by '
                                             'chatbot_data_reference.py, using own framework. Also see training.py.')
...
parser.add_argument(
    '--encoder-state-size',
    type=int, required=False, default=1600,
...
parser.add_argument(
    '--dropout',
    type=float, required=False, default=0.35,
    help='Drop out rate')
...
use_mixed_precision = args.float16 == True
...
metric_names = {
    ...
    'duration.mean.batch': 'training_time',
    'duration.epoch': 'training_time',
    'batch_size': 'size'
}
...
def load_sentence_pair_data(sentence_pair_file):
    logger.info(f'\nLoading sentence pair file : {sentence_pair_file}\n')
...
training_dataset, voc = load_sentence_pair_data(dataset_path_for('training'))
logger.info('Loading validation set ...')
validation_dataset, _unused_ = load_sentence_pair_data(dataset_path_for('validation'))
...
class TeacherForcingController(Callback):
...
class ChatbotTrainer(DefaultTrainer):
...
def create_loss_eval_func(trainer):
    def calc_loss(batch):
        with torch.no_grad():
            loss, _ = trainer.evaluate_loss(batch,
                                            inference_mode=True,
                                            evaluate_settings={'use_teacher_forcing': True})
...
callbacks = [TeacherForcingController(name="TeacherForcingController"),
             TestMetricsLogger(validation_dataset_loader,
                               'validation',
                               {
                                   'loss': create_loss_eval_func(trainer)
                               }),
             ]
...
if is_first_worker:
    callbacks += [
        BatchSizeLogger(),
        CheckpointManager(metric_to_monitor='validation.mean.loss',
                          base_checkpoint_filename=args.experiment_name,
                          archive_last_model_checkpoint_every=20000),
        LogProgress(log_period=progress_logging_period, set_names=["training", "validation"]),
        AutoTensorboard(experiment_name=experiment_name, dataset_name='training', metric_names=metric_names),
        AutoTensorboard(experiment_name=experiment_name, dataset_name='validation', metric_names=metric_names),
        # Batch-level batch duration and batch size
        Tensorboard(['duration.batch', 'batch_size'],
                    experiment_name=experiment_name,
                    dataset_name='training_params',
                    metric_names=metric_names,
                    ignore_missing_metrics=True),
        # Batch-level average batch duration
        Tensorboard(['duration.mean.batch'],
                    experiment_name=experiment_name,
                    dataset_name='training_params',
...
manager = TrainingManager(trainer,
...
if tc_file:
    ...
    # TODO : handle syncing when loading and saving in the distributed case
    #        Likely, loading a checkpoint this late doesn't work in the distributed case
    if distributed:
        dist.barrier()
